...
export class LoadManager extends EventEmitter {
  ...
  constructor(config: LoadManagementConfig, citrineWsUrl: string, defaultMaxPowerKw = 22) {
    ...
    this.citrineWs = new WebSocket(citrineWsUrl);
  ...
  public setStationChargingLimit(stationId: string, maxPowerKw: number): boolean {
    ...
    station.maxPower = Math.max(station.maxPower, maxPowerKw);
    // Keep maxPower as hardware/config ceiling; profile limit is the operational cap
  ...
  private checkLoadAdjustment(): void {
    ...
    if (totalPower > this.config.maxSitePower - this.config.adjustmentThreshold) {
      console.log(`Load approaching limit (${this.config.maxSitePower}kW). Adjusting charging profiles...`);
      setTimeout(() => {
        this.adjustChargingProfiles(totalPower);
  ...
  private adjustChargingProfiles(currentTotalPower: number): void {
    // Only adjust if we're still over the threshold
    ...
    const excessPower = currentTotalPower - (this.config.maxSitePower - this.config.adjustmentThreshold);
    const reductionPerStation = excessPower / activeStations.length;
    
    console.log(`Adjusting ${activeStations.length} active stations, reducing ${reductionPerStation}kW per station`);
    
    // Send SetChargingProfile commands to each active station
    for (const station of activeStations) {
      const newMaxPower = Math.max(0, station.currentPower - reductionPerStation);
  ...
  private setupWebSocketHandlers(): void {
    this.citrineWs.on('open', () => {
      console.log('Connected to CitrineOS WebSocket');
    });
    
    this.citrineWs.on('message', (data) => {
      try {
    ...
    this.citrineWs.on('error', (error) => {
      console.error('WebSocket error:', error);
    });
    
    this.citrineWs.on('close', () => {
  ...
  private handleCitrineMessage(message: any): void {
    ...
    } else if (action === "StartTransaction" || action === "TransactionEvent") {
      // TransactionEvent (OCPP 2.x) may include Started/Ended; handle both
      const eventType = message?.payload?.eventType || message?.payload?.triggerReason;
      if (action === "StartTransaction" || String(eventType).toLowerCase() === "started") {
        this.handleStartTransaction(message);
      } else if (String(eventType).toLowerCase() === "ended") {
        this.handleStopTransaction(message);
      }
    } else if (action === "StopTransaction") {
      this.handleStopTransaction(message);
    } else if (action === "BootNotification" || action === "StatusNotification") {
      const stationId = message?.stationId || message?.payload?.stationId || message?.payload?.chargingStationId;
  ...
  private handleMeterValues(message: any): void {
    const { connectorId, meterValue } = message.payload;
    const stationId = message.stationId; // Assuming this is available in the message
    
    // Extract power value from meterValue
    let powerValue = 0;
